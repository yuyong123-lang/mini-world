---
文档编号: DOC-TECH-03-001
标题: 协作规则与 DoD（对所有 Agent 生效）
类型: 规范
状态: 现行
日期: 2026-09-01
作者: my-world 项目组
描述: 多 Agent 协作规则与 DoD：文件所有权、契约先行、缺陷处理、集成冒烟与提交流程
标签: [协作规范, DoD, 多Agent]
---
...
## 2. 契约优先
- 所有公开接口以 [interfaces.md](../05-接口文档/interfaces.md) 为准，不得更改导出名/参数形状
...
- 每个区域 agent 恰好 **3 个独占文件**：`src/data/regions/parts/<组>.ts` + `src/world/buildings/<组>.ts` + `tests/regions/<组>.test.ts`（组名对照见 02-技术层/05-接口文档/buildings.md §6）
...
- `parts/<组>.ts` 导出组对象（`Partial<Record<RegionId, RegionDef>>`：地形参数/树表/结构表/动物权重/氛围色）；`buildings/<组>.ts` 导出本组 stamp 函数（签名 `(ax, az, fy, heightAt, put)`，只用 kit 工具，铁律见 02-技术层/05-接口文档/buildings.md §3）
...
- 需要改冻结文件 → 停止，**走主线程契约修订**：报告问题 → 主线程修订 02-技术层/05-接口文档/buildings.md / interfaces.md → 重发受影响任务（同 §2 契约优先流程）
